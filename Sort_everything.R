...
move_file <- function(the_file, the_year) {
  file.rename(from=paste0("Data/", the_file), to=paste0("Data/", the_year, "/", the_file))
}

get_file_list_from_csv <- function(the_year) {
  the_csv <- paste(the_year, "_data_table.csv", sep="")
  read_csv(paste(here(), datadir, the_csv, sep="/")) %>%
    clean_colnames()
}
...
create_path <- function(the_stem) {
  ...
  path <- paste(here(), datadir, the_file, sep="/")
  return(path)
}
...
find_file <- function(the_stem, the_year) {
    if(file.exists(create_path(the_stem))) {
      ...
      if(file.exists(paste(here(), datadir, the_year, create_file_name(the_stem), sep="/"))) {
...
set_title <- function(the_year) {
  tx  <- readLines(paste(here(), datadir, "IPEDSpublic", "header.yml", sep="/"))
...
#      tx2  <- gsub(pattern = "Departmental", replace = paste("Departmental", target_active, sep="_"), x = tx2)
  writeLines(tx2, con=paste(here(), datadir, "IPEDSpublic", "year.qmd", sep="/"))
  rest <- readLines(paste(here(), datadir, "IPEDSpublic", "year_template.qmd", sep="/"))
  write(rest,file=paste(here(), datadir, "IPEDSpublic", "year.qmd", sep="/"),append=TRUE)
}
